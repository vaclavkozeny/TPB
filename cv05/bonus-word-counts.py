...
input = sc.textFile("/files/articles-clear.jsonl")
contents = input.map(lambda line: json.loads(line).get("content", ""))
words = contents.flatMap(lambda x: filter(None, re.split(r'\W+', x.lower())))
